...
const COMPACT_LOCK_STALE_MS = 6e4;
...
*/
const DEEPSEEK_OFFICIAL_CONFIG = {
...
function readProviderConfigs() {
	const configs = [];
	try {
		const providers = parseSimpleYaml(readSettingsDocument())["llm-pi-ai"]?.["providers"] ?? {};
...
function readLanHosts() {
	try {
		const declared = parseSimpleYaml(readSettingsDocument())["stats-panel"]?.["lanHosts"];
...
function readBucketOffsetMinutes() {
	...
	let declared;
	try {
		declared = parseSimpleYaml(readSettingsDocument())["stats-panel"]?.["dayBoundary"];
	} catch {
		return local;
	}
...
function parseSimpleYaml(text) {
	...
	for (const line of text.split("\n")) {
		...
		} else parent[key] = stripQuotes(value);
	}
	return root;
}
...
*/
const OPENCODE_GO_PROVIDERS = /* @__PURE__ */ new Set(["opencode-go", "opencode-go-bridge"]);
/**
...
async function probeChannel(ctx, config, resolveKey) {
	...
	if (config.provider === "mimo" || url.includes("token-plan-cn.xiaomimimo.com")) {
		...
		if (primary !== void 0 && typeof primary === "object" && primary !== null) {
	...
	if (url.includes("api.openai.com")) {
		...
		const usage = await Promise.all(buckets.map(async (bucket) => {
			...
			const rows = (await probeJson(`https://api.openai.com/v1/usage?start_time=${Math.floor(windowStartMs / 1e3)}&bucket_width=${bucket.widthMs === day ? "1d" : "1h"}`, { authorization: `Bearer ${key}` }))["data"] ?? [];
			let input = 0;
			let output = 0;
			let approximate = false;
			for (const row of rows) {
				const rowStartMs = typeof row["start_time"] === "string" ? Date.parse(row["start_time"]) : NaN;
...
function flushAppendQueue() {
	return appendTail.then(() => appendQueue.length > 0 ? flushAppendQueue() : void 0);
}
...
*/
function acquireCompactionLock() {
...
function releaseCompactionLock() {
	try {
		unlinkSync(COMPACT_LOCK_FILE);
	} catch {}
}
...
function apply(ctx) {
	...
	*/
	...
	for (const record of records) {
		...
		liveRoutes.set(record.sessionId, {
			model: record.model,
	...
	*/
	const maybeCompact = async (persistState) => {
	...
	const balancesRoute = {
		...
		handler: async (req, res) => {
			...
			if (balancesInFlight === void 0) {
				...
				if (!seen.has(DEEPSEEK_OFFICIAL_CONFIG.provider) && await resolveKey(DEEPSEEK_OFFICIAL_CONFIG.apiKeyEnv) !== void 0) configs.push(DEEPSEEK_OFFICIAL_CONFIG);
...
//#endregion
export { aggregateOf, apply, compactRecords, computeSummary, flushAppendQueue, inject, isStatsRequestAllowed, mergeAggregates, name, parseSimpleYaml, probeChannel };
