...
function isPrivateAddress(address) {
    ...
    if (peer.startsWith('fe8') || peer.startsWith('fe9') || peer.startsWith('fea') || peer.startsWith('feb'))
        return true;
 ...
 */
export function isStatsRequestAllowed(request, lanHosts = []) {
    ...
    if (loopback) {
        ...
        if (!isPrivateAddress(address))
            return false;
        if (!lanHosts.some(entry => normalizeHostname(entry) === hostname))
            return false;
    }
...
function readLanHosts() {
    try {
        const root = parseSimpleYaml(readFileSync(SETTINGS_PATH, 'utf8'));
        const panel = root['stats-panel'];
        ...
        if (Array.isArray(declared)) {
            return declared.filter((entry) => typeof entry === 'string' && entry.trim() !== '').map(entry => entry.trim());
        }
    }
    catch {
...
function readBucketOffsetMinutes() {
    ...
    try {
        const root = parseSimpleYaml(readFileSync(SETTINGS_PATH, 'utf8'));
        declared = root['stats-panel']?.['dayBoundary'];
    }
    catch {
        return local;
    }
...
async function probeJson(url, headers, timeoutMs = 10_000) {
...
async function probeNewApiBilling(config, base, key, now, resolveKey, diag = {}) {
    ...
    try {
        ...
        if (isNetworkProbeError(e))
            diag.note = `上游查询失败：${e instanceof Error ? e.message : String(e)}`;
        return undefined;
    }
    ...
    try {
        ...
        const usage = await probeJson(`${base}/dashboard/billing/usage?start_date=${fmt(start)}&end_date=${fmt(end)}`, { authorization: `Bearer ${key}` }, 12_000);
    ...
    if (totalUsd >= NEWAPI_UNLIMITED_SENTINEL) {
        ...
        if (session !== undefined) {
            ...
            if (session.newApiUser !== undefined)
                headers['new-api-user'] = session.newApiUser;
            try {
...
async function probeSub2ApiUsage(config, base, key, now, diag = {}) {
    ...
    const noteParts = [];
    if (typeof body['planName'] === 'string' && body['planName'] !== '')
        noteParts.push(body['planName']);
    ...
    if (today !== undefined) {
        ...
        if (tok !== undefined)
            noteParts.push(`今日 ${tokensShort(tok)} tok / ${reqs ?? 0} 次`);
    }
    if (total !== undefined) {
        const cost = numField(total, 'actual_cost') ?? numField(total, 'cost');
        if (cost !== undefined)
            noteParts.push(`累计成本 $${amountText(cost)}`);
    }
    return {
        channel: config.provider,
        kind: 'balance',
...
async function probeChannel(ctx, config, resolveKey) {
    ...
    if (config.provider === 'mimo' || url.includes('token-plan-cn.xiaomimimo.com')) {
        ...
        if (quota.length === 0)
            throw new Error('MiMo 平台未返回套餐用量');
        return { channel: config.provider, kind: 'plan', displayName: config.displayName, quota, fetchedAt: now };
    }
...
function countField(object, field, optional = false) {
    if (object[field] === undefined && optional)
        return 0;
...
function sumAggregateRows(rows) {
    ...
    for (const row of rows) {
        for (const key of AGGREGATE_COUNTER_KEYS) {
            ...
            if (!isNonNegativeSafeInteger(next))
                return null;
            sum[key] = next;
        }
    }
...
function normalizeUsageCounters(value) {
    ...
    if (inputTokens === null || outputTokens === null || cacheReadTokens === null
        ...
        return null;
    if (tokenTotalOrNull(inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens) === null)
        return null;
...
function normalizeModelStats(value) {
    const object = objectOf(value);
    if (object === null || typeof object.model !== 'string')
        return null;
...
function normalizeChannelStats(value) {
    ...
    if (object === null || typeof object.channel !== 'string' || !Array.isArray(object.models)
        || !object.models.every(model => typeof model === 'string'))
        return null;
    const calls = countField(object, 'calls');
    const inputTokens = countField(object, 'inputTokens');
    const outputTokens = countField(object, 'outputTokens');
...
function normalizeAggregate(value) {
    ...
    if (object === null || totalsObject === null || !Array.isArray(object.modelStats)
        || !Array.isArray(object.channelStats) || !Array.isArray(object.dailyStats))
        return null;
    ...
    if (normalizedWeekly.some(value => value === null) || normalizedMonthly.some(value => value === null))
        return null;
    ...
    const totals = { calls, inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens, reasoningTokens };
    const dimensions = [normalizedModel, normalizedChannel, normalizedDaily];
    if (hasWeeklyStats)
        dimensions.push(normalizedWeeklyStats);
    if (hasMonthlyStats)
        dimensions.push(normalizedMonthlyStats);
...
function loadArchive() {
    try {
        ...
        if (parsed?.['version'] !== 1 || !isValidTimestamp(parsed['cutoffTs']))
            return null;
    ...
    try {
        if (existsSync(BACKFILL_STATE_FILE)) {
            ...
            if (parsed?.['version'] === 2 && objectOf(revisions) !== null
                && Object.values(revisions).every(value => typeof value === 'string')
...
function foldAggregate(fold, aggregate) {
    ...
    for (const key of ['dailyStats', 'weeklyStats', 'monthlyStats']) {
...
export function aggregateOf(records, offsetMinutes = 0) {
    const fold = newFold();
    for (const record of records)
        foldRecord(fold, record, offsetMinutes);
...
export function mergeAggregates(a, b) {
    const fold = newFold();
 ...
 */
export function compactRecords(records, now, offsetMinutes = 0) {
    if (records.length === 0 || !isValidTimestamp(now))
        return null;
 ...
 */
export function computeSummary(records, archive, options = {}) {
    const offsetMinutes = options.offsetMinutes ?? 0;
    const fold = newFold();
    for (const record of records)
        foldRecord(fold, record, offsetMinutes);
    if (archive !== undefined)
        foldAggregate(fold, archive);
...
export function apply(ctx) {
    ...
    const summaryCache = { value: undefined, dirty: true };
    /** Per-application balance cache and probe deduplication. */
    const balancesCache = { value: undefined,
        get() { return this.value; },
        set(balances) { this.value = { at: Date.now(), balances }; },
    };
    ...
    const collect = (sessionId, seq, model, provider, usage, ts) => {
        ...
        if (typeof sessionId !== 'string' || !isNonNegativeSafeInteger(seq) || !isValidTimestamp(ts) || sessionId.trim() === '')
            return;
        if (archive !== null && ts < archive.cutoffTs)
            return;
    ...
    void (async () => {
        try {
            ...
            for (const entry of sessions) {
                ...
                if (!live && revision !== undefined && stateUsable && revisions[id] === revision)
                    continue;
                try {
            ...
            const persistBackfillState = () => {
                if (snapshots === null)
                    return;
                try {
                    writeFileAtomic(BACKFILL_STATE_FILE, JSON.stringify({ version: 2, revisions, recordsAtWrite: records.length }));
                }
                catch {
                    // Next boot rechecks revisions — safe, just slower.
                }
