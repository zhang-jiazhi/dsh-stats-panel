...
function foldRecord(fold, record) {
...
export function aggregateOf(records) {
    const fold = newFold();
    for (const record of records)
        foldRecord(fold, record);
 ...
 */
export function compactRecords(records, now) {
 ...
 */
export function computeSummary(records, archive) {
    const fold = newFold();
    for (const record of records)
        foldRecord(fold, record);
        ...
        try {
            ...
            if (records.length >= maxRecords) {
                ...
                if (plan !== null) {
                    ...
                    const nextArchive = { version: 1, cutoffTs: plan.cutoffTs, aggregate };
