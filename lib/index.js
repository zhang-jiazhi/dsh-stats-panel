...
*/
function compactRecords(records, now) {
...
*/
function computeSummary(records, archive) {
	const fold = newFold();
	for (const record of records) foldRecord(fold, record);
	...
	const cacheHitRate = totalTokens > 0 ? (fold.totals.cacheReadTokens + fold.totals.cacheWriteTokens) / totalTokens * 100 : 0;
	return {
...
function apply(ctx) {
	...
	ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			if (summaryCache.dirty || summaryCache.value === void 0) {
